...
public class MainActivity extends AppCompatActivity {
    ...
    public void onRunModelSuccessed() {
        ...
        if (configParser.getModelType().equalsIgnoreCase("segmenter")) {
            ...
            for(int i=0;i<detResult.getBoxes().size();i++) {
                if(detResult.getBoxes().get(i).getScore()<0.5){
                    detResult.getBoxes().remove(i);
                }
            }
            int totalNum =  detResult.getBoxes().size();
            ...
            tvOutputResult.setText("total number of rebar: "+String.format("%d", totalNum));;
